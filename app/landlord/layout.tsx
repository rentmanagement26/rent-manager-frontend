    ...
    <SidebarProvider>
      <PageHeaderProvider>
        <div className="lg:grid lg:grid-cols-[256px_1fr] lg:grid-rows-[auto_1fr] lg:h-screen lg:overflow-hidden bg-slate-50 text-slate-900">
          <LandlordSidebar role={session.role || "Landlord"} email={session.email} />

          <div className="hidden lg:flex lg:col-start-2 lg:row-start-1 items-center border-b border-slate-200 bg-slate-50 px-8">
            <PageHeaderSlot />
          </div>

          <main className="h-screen overflow-y-auto overscroll-y-none px-8 pb-8 lg:pt-8 lg:h-auto lg:col-start-2 lg:row-start-2">
            <div className="lg:hidden pt-8 -mx-8 px-8 mb-6 border-b border-slate-200 bg-slate-50">
